...
| 4 | Автокомплит не работает для кастомных полей с кириллическими / многословными именами (`"Мои задачи"` и т.п.) | Пользователь набирает `"Мо` — popup либо показывает весь список без фильтра, либо вставка ломает текст (`"Мо"Мои задачи"`) | (a) Backend `search.suggest.position.ts`: токенизатор кидает `UNTERMINATED_STRING` на недописанной кавычке → `catch` возвращал `emptyField()` с пустым prefix; (b) frontend `ttql-completion.ts`: `IDENT_RE = /[\w."-]*/` без флага `u` — `\w` ASCII-only, поэтому `matchBefore` не захватывает кириллицу (неверный `from`), а `validFor` закрывал popup на первом же не-ASCII символе. | (a) В `analysePosition` добавлен recovery-путь `recoverFromUnterminatedString`: режет по последней открывающей кавычке, токенизирует префикс-часть, оставляет typed suffix как `prefix`; (b) `IDENT_RE` переведён на Unicode-property escape `/[\p{L}\p{N}_."-]*/u`. Regression-тест: «Cyrillic multi-word custom field name in unterminated quotes → field context with prefix». |

**Покрытие тестами:** 488 backend tests pass (parser suite, включая 7 новых в compiler, 2 новых в suggest и 1 новый для Cyrillic recovery; обновлён существующий `unterminated string → graceful fallback` — теперь возвращает `value`-контекст с сохранённым prefix'ом). Frontend — typecheck clean. Интеграционные/e2e fixtures для новой column-picker UX и Cyrillic-autocomplete — follow-up (см. TTSRH-39 ниже).
