...
**Статус:** IN_PROGRESS — реализация начата с TTBUS-0; первый фундаментальный инкремент смержен в main через PR #170 (2026-04-28)
...
**Текущий прогресс:** TTBUS-0 начат. Закрыта часть фундамента event bus: Prisma-модели `event_outbox` / `processed_messages`, envelope schema, `publishInTx(...)`, consumer-side dedup helper и тесты для этого слоя. Kafka relay, kafkajs wrapper, docker-compose и интеграция producer'ов в issues-модуль ещё не выполнены.
