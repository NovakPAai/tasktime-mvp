...
- Обновление кода: `git pull`, установка зависимостей в `backend`, перезапуск сервиса (см. [DEPLOY.md](../DEPLOY.md)).
- После обновления при необходимости примените изменения схемы БД (запуск `schema.sql` или отдельные миграции).
